...
content = content.replace(
  /if \(viewportCenter < linceCenter\) \{\s*const p = progressBetween\(introCenter, linceCenter\);\s*weights = \[1 - p, p, 0\.1, 0\]; \/\/ Adiciona um leve overlap das outras cores\s*\} else if \(viewportCenter < bungeCenter\) \{\s*const p = progressBetween\(linceCenter, bungeCenter\);\s*weights = \[0, 1 - p, p, 0\.1\];\s*\} else if \(viewportCenter < mapfitCenter\) \{\s*const p = progressBetween\(bungeCenter, mapfitCenter\);\s*weights = \[0\.1, 0, 1 - p, p\];\s*\} else \{\s*const p = progressBetween\(mapfitCenter, outroCenter\);\s*weights = \[p, 0\.1, 0, 1 - p\];\s*\}/g,
...
content = content.replace(
  /layers\.forEach\(\(layer, index\) => \{\s*\/\/ Reduz a intensidade das camadas coloridas \(índices 1, 2, 3\) para não ofuscar o texto\s*const finalWeight = index === 0 \? weights\[index\] : \(weights\[index\] \?\? 0\) \* 0\.6;\s*layer\.style\.opacity = `\$\{finalWeight\}`;\s*layer\.style\.transition = "opacity 0\.6s cubic-bezier\(0\.23, 1, 0\.32, 1\)";\s*\}\);/g,
...
content = content.replace(
  /\{\/\* Mapfit Scene - Auras de Ouro\/Escuro \*\/\}\s*<div className="journey-bg-scene absolute inset-0 opacity-0 bg-\[radial-gradient\(circle_at_50%_50%,rgba\(215,162,58,0\.08\),transparent_60%\),radial-gradient\(circle_at_10%_90%,rgba\(26,26,26,0\.05\),transparent_40%\),white\]" \/>/g,
...
content = content.replace(
  /<div ref=\{lineRef\} className="h-0 w-full origin-top bg-\[#ff6a00\]" \/>\s*<div ref=\{orbRef\} className="absolute -left-\[4px\] top-0 z-20 h-3 w-3 rounded-full border-2 border-\[#ff6a00\] bg-white shadow-\[0_0_8px_rgba\(255,106,0,0\.4\)\]" \/>/g,
...
content = content.replace(
  /<div className="absolute left-4 z-10 h-3 w-3 -translate-x-1\/2 rounded-full border-2 border-\[#ff6a00\] bg-white sm:left-1\/2" \/>/g,
  `<div className="absolute left-4 z-10 h-3 w-3 -translate-x-1/2 rounded-full border-2 border-black/10 bg-white sm:left-1/2" />`
);

content = content.replace(
  /\) : i === 3 \? \(\s*<div data-journey-scene="0">\s*<StudioFeaturedCard item=\{item\} \/>\s*<\/div>\s*\) : i === 5 \? \(\s*<div data-journey-scene="0">\s*<AvantFeaturedCard item=\{item\} \/>\s*<\/div>/g,
  `) : i === 3 ? (
